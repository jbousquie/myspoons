class MyChart {
    ...
    static ENERGY_ON_TIME = 30;
    static TIME_SCALE = 40;
    static TIMESERIES_SCALE = 41;
    ...
    getDataForLineEnergy() {
        ...
        dataArray.forEach(row => {
            ...
            this.data.push(rate);
            let commentLabels = this.getWordsFromComments(row[5]);
            this.customLabels.push(commentLabels);
        })
    }
    ...
    displayLineEnergy(scaleType) {
        let scale = 'time';
        let tension = 0.1;
        switch (scaleType) {
            case MyChart.TIME_SCALE:
                scale = 'time';
                tension = 0.1;
                break;
            case MyChart.TIMESERIES_SCALE:
                scale = 'timeseries';
                tension = 0.3;
        }
        this.getDataForLineEnergy();
        var chart = new Chart(
            this.ctx,
            {
                type: 'line',
    ...
    display(chartType, labelString, title, description) {
        ...
        switch (chartType) {
            case 1:
                chart = this.displayBarEvents(MyChart.MIXED);
                break;
            case 2:
                chart = this.displayBarEvents(MyChart.BY_FREQUENCY);
                break;
            case 3:
                chart = this.displayBarEvents(MyChart.BY_INTENSITY);
                break;
            case 4:
                chart = this.displayPyramidPeriod(MyChart.PERIOD_WEEKDAY, MyChart.AVG_BY_PERIOD, true);
                break;
            case 5:
                chart = this.displayPyramidPeriod(MyChart.PERIOD_WEEKDAY, MyChart.COUNT_BY_PERIOD, true);
                break;
            case 6:
                chart = this.displayPyramidPeriod(MyChart.PERIOD_HOUR, MyChart.AVG_BY_PERIOD, false);
                break;
            case 7:
                chart = this.displayPyramidPeriod(MyChart.PERIOD_HOUR, MyChart.COUNT_BY_PERIOD, false);
                break;
            case 8:
                chart = this.displayLineEnergy(MyChart.TIME_SCALE);
                break;
            case 9:
                chart = this.displayLineEnergy(MyChart.TIMESERIES_SCALE);
                break;
        }
