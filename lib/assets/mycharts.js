class MyChart {
    ...
    // Returns a sorted array (by count desc) of {label: string, count: int, avg: float}
        ...
        deltas.forEach(row => {
            ...
            labels.forEach(label => {
                if (!(label in frequencies)) {
                    frequencies[label] = {count: 0, avg: 0, label: label};
                }
